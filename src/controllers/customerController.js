...
// ─────────────────────────────────────────────
//  GET ALL CUSTOMERS  (paginated + search)
//  ✅ UPDATED: now also returns each customer's last payment
//  (amount, method, bank_name, date) in a single query pass —
//  no N+1 per-customer ledger calls needed on the Flutter side.
// ─────────────────────────────────────────────
exports.getAllCustomers = async (req, res) => {
  try {
    const { search, page = 1, limit = 20, active, customer_type } = req.query;
    ...
    // Recalculate balances from ledger for each customer, and grab their
    // most recent payment entry in the same pass (one ledger query per
    // customer, but it was already being made for the balance calc).
    const customersWithUpdatedBalances = await Promise.all(
      customers.map(async (customer) => {
        // Get all ledger entries for this customer (excluding uncleared cheques if needed)
        const entries = await CustomerLedger.findAll({
        ...
        // ── NEW: fetch the most recent payment entry for this customer ──
        ...
        // Return customer with updated balance + last payment
        return {
          ...customer.toJSON(),
          balance: parseFloat(calculatedBalance.toFixed(2)),
